...
def Kubo(a,vpp_pi,m,T):		#UNIDADES EN eV
	...
	def v(kx, ky):
		...
		'''
		return np.array([[0, 1j*vpp_pi*a*(exp(1j*a*kx) - 1/2 * exp(1j*((-a/2)*kx - ((sqrt(3)*a)/2) * ky)) - 1/2*exp(1j*((-a/2)*kx + ((sqrt(3)*a)/2) * ky)))],
	                    [ 1j*vpp_pi*a*(-exp(-1j*a*kx) + 1/2 * exp(-1j*((-a/2)*kx - ((sqrt(3)*a)/2) * ky)) + 1/2*exp(-1j*((-a/2)*kx + ((sqrt(3)*a)/2) * ky))), 0]])
		'''
